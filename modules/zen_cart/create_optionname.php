<?
	$db->select("webegg_com_au_$username");
	
	$sqlqry		    = "SELECT * from products_options_types";
	$gettypes 		= $db->get_results($sqlqry);
	
	$attribute_id = $_REQUEST["id"];
	
	if($_POST['products_options_name']!='') {
		extract($_REQUEST);

		if($hdattributeid=='') {
			$db->query("INSERT INTO products_options (products_options_name, products_options_type, products_options_sort_order) VALUES ('$products_options_name', '$products_options_type','$order')");
			$update_message = '<li class="green"><span class="ico"></span><strong class="system_title">Attributes successfully created!</strong></li>';
		} else {
			$db->query("UPDATE products_options SET products_options_name = '$products_options_name', products_options_type = '$products_options_type' , products_options_sort_order='$order' WHERE products_options_id ='$hdattributeid'");
			$update_message = '<li class="green"><span class="ico"></span><strong class="system_title">Attributes successfully updated!</strong></li>';
		}	
	}
	
	if(!empty($attribute_id)) {
		$sqlqry		    = "SELECT * from products_options where products_options_id ='".$attribute_id."' ";
		$getoptions 	= $db->getRow($sqlqry);
	}


?>

<!--[if !IE]>start section<![endif]-->	
<div class="section">
    <!--[if !IE]>start title wrapper<![endif]-->
    <div class="title_wrapper">
        <h2>
        <?php
     if ($id == !'') { ?>
         Edit
     <?php } else { ?>
		 Add New
	<?php  } ?>
         Option Name</h2>
        
        <!--[if !IE]>start section menu<![endif]-->
        <!--<ul class="section_menu">
            <li><a href="#"><span><span>Inactive Tab</span></span></a></li>
            <li><a href="#" class="active"><span><span>Active Tab</span></span></a></li>
            <li><a href="#"><span><span>Products</span></span></a></li>
            <li><a href="#"><span><span>Last One</span></span></a></li>
        </ul>-->
        <!--[if !IE]>end section menu<![endif]-->
        
        
        <span class="title_wrapper_left"></span>
        <span class="title_wrapper_right"></span>
    </div>
    <!--[if !IE]>end title wrapper<![endif]-->
    <!--[if !IE]>start section content<![endif]-->
    <div class="section_content">
        <!--[if !IE]>start section content top<![endif]-->
        <div class="sct">
            <div class="sct_left">
                <div class="sct_right">
                    <div class="sct_left">
                        <div class="sct_right">
                                                        
                            <!--<p>
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec blandit, nisl sit amet iaculis ullamcorper, orci tellus feugiat est, at dapibus massa dui vel lectus. Sed felis nunc, pharetra ullamcorper, fermentum nec, cursus nec, ipsum. Nunc porta blandit risus. Proin pharetra. Proin ultrices viverra lorem. Phasellus tellus enim, accumsan et, luctus vitae, mattis in, diam. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec vulputate, arcu consectetur auctor lacinia, justo est suscipit massa, a 
                            </p>-->
                            
                            
                            <!--[if !IE]>start forms<![endif]-->
							<ul class="system_messages">
								<?php echo "$update_message"; ?>
							</ul>
                            <form action="" class="search_form general_form" method="post">
								<input type="hidden" id="hdattributeid" name="hdattributeid" value="<?php echo $getoptions->products_options_id; ?>" />
                                <!--[if !IE]>start fieldset<![endif]-->
                                <fieldset>
                                    <!--[if !IE]>start forms<![endif]-->
                                    <div class="forms" id="attributes">
                                    <!--[if !IE]>start row<![endif]-->
                                    <div class="row" >
                                        <label>Products Option Name:</label>
                                        <div class="inputs">
                                            <span class="input_wrapper">
											<input class="text" type="text" id="products_options_name" name="products_options_name" value="<?php echo $getoptions->products_options_name?>" size="50"/></span>
                                            
                                        </div>
                                    </div>
                                    <!--[if !IE]>end row<![endif]-->
                                    
                                    <!--[if !IE]>start row<![endif]-->
                                    <div class="row" >
                                        <label>Option Type :</label>
                                        <div class="inputs">
                                            <select name="products_options_type" id=""> 
												<? for($k=0;$k<count($gettypes);$k++) { ?>
													<option value="<? echo $gettypes[$k]->products_options_types_id; ?>" <?php if($getoptions->products_options_type==$gettypes[$k]->products_options_types_id) { ?>selected="selected" <? } ?> >  <? echo $gettypes[$k]->products_options_types_name; ?></option>				
												<? } ?>									
											</select>
                                            
                                        </div>
                                    </div>
                                    <!--[if !IE]>end row<![endif]-->
									<!--[if !IE]>start row<![endif]-->
                                    <div class="row" >
                                        <label>Sort Order:</label>
                                        <div class="inputs">
                                           <select name="order" id="order">
										   	 <?php for($j=1;$j<=20;$j++) { ?>
												<option value="<?php echo $j; ?>" <?php if($j==$getoptions->products_options_sort_order) { ?> selected="selected" <? } ?>><? echo $j;?></option>
											<?php  } ?>	
											</select>
                                        </div>
                                    </div>
                                    <!--[if !IE]>end row<![endif]-->							
                                    <!--[if !IE]>start row<![endif]-->
                                    <div class="row">
                                        <div class="buttons" style="padding-left:200px;">
                                            
                                            
                                            <ul>
                                                <li><span class="button send_form_btn"><span><span>SUBMIT</span></span><input name="submit" type="submit" /></span></li>
                                                <li><span class="button cancel_btn"><span><span>CANCEL</span></span><input name="" onclick="javascript:history.back()" /></span></li>

                                            </ul>
                                            
                                           
                                        </div>
                                    </div>
                                    <!--[if !IE]>end row<![endif]-->
                                    
                                    
                                    
                                    </div>
                                    <!--[if !IE]>end forms<![endif]-->
                                    
                                </fieldset>
                                <!--[if !IE]>end fieldset<![endif]-->
                                
                                
                                
                                
                            </form>
                            <!--[if !IE]>end forms<![endif]-->	
                            
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!--[if !IE]>end section content top<![endif]-->
        <!--[if !IE]>start section content bottom<![endif]-->
        <span class="scb"><span class="scb_left"></span><span class="scb_right"></span></span>
        <!--[if !IE]>end section content bottom<![endif]-->
        
    </div>
    <!--[if !IE]>end section content<![endif]-->
</div>
<!--[if !IE]>end section<![endif]-->
