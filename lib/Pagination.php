...
class Pagination {
    ...
    function paginate() {
            ...
            if($nbItems<=$this->limit)
            {
                    ...
                    if ($allPages>9) {
                        if ($currentPage>=1&&$currentPage<=$allPages) {
                            ...
                            for($i=$minPages-4; $i<$maxPages+5; $i++) {
                                $pagination .= ($i == $currentPage) ? "<a href=\"#\" class=\"current\">".$i."</a> " : "<a href=\"".$this->filePath."?start=".(($i-1)*$this->limit).$this->otherParams."\">".$i."</a> ";
                            }
                ...
                if ($currentPage>1)
                {
                    $pagination = "<a href=\"".$this->filePath."?start=".$this->otherParams."\">".$this->first_r."</a>"
                                . " <a href=\"".$this->filePath."?start=".(($currentPage-2)*$this->limit).$this->otherParams."\">"
                                . "".$this->previous_r."</a> ".$pagination;
                }
                if ($currentPage<$allPages)
                {
                    $pagination .= "<a href=\"".$this->filePath."?start=".($currentPage*$this->limit).$this->otherParams."\">"
                                 . "".$this->next_r."</a> <a href=\"".$this->filePath."?start=".(($allPages-1)*$this->limit).$this->otherParams."\">"
                                 . "".$this->last_r."</a>";
                }
